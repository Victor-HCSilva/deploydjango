...
class GestaoDePacientesView(FormHandlerView):
  def get_context_data(self):
     ...
     cpf_filter = self.request.GET.get('cpf_search') or ''
     medico_cpf_filter = self.request.GET.get('medico_cpf_search') or ''
     paciente_cpf_filter = self.request.GET.get('paciente_cpf_search') or ''

     all_pacientes = self.handle_search(all_pacientes, search_query, "nome")
     all_pacientes = self.handle_search(all_pacientes, cpf_filter, "cpf")
    ...
    def get_context_data(self):
        ...
        if self.request.method == 'POST':
            ...
            if paciente:
                ...
                self.context = {'form': self.form, 'paciente': paciente, 'erro': 'Formulário inválido. Verifique os campos.'}
